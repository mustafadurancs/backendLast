...
public interface SchoolRepository extends JpaRepository<School, Long> {
    ...
    @Query(value = "SELECT * FROM School s WHERE s.GPA <=:gpa and  s.SATRange_Min <=:sat and  s.SATRange_Max >=:sat and s.ACTRange_Min <=:act and  s.ACTRange_Max >=:act and s.hbcu =:hbcu and s.type =:type and s.two_four_year LIKE %:schoolYear%  order by s.score", nativeQuery = true)
